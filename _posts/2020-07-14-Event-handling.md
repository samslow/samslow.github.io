...
![](https://mblogthumb-phinf.pstatic.net/20160225_197/zzingko2273_1456380245455vLKy3_JPEG/%BF%B5%C8%AD%BD%C5%BC%BC%B0%E8%C8%B2%C1%A4%B9%CE%BC%B1%B1%DB%B6%F3%BD%BA%C5%A9%B7%D2%C7%CF%C3%F7%BC%B1%B1%DB%B6%F3%BD%BAGRANDBEAST.jpg?type=w2)
...
여기서는 이런 방식의 코드가 낫겠지만, `<ul>`과 `<ol>` 하위의 `<li>` 를 넣고 여기에도 이벤트를 달아주어야 하는 상황이라면

`<ul>`과 `<ol>` 에서 이를 관리하도록 하고 `<li>` 는 이벤트를 신경쓰지 않아도 되는 가장 바람직한 코드 일 것이다.
